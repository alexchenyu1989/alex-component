...
export default function AdminArticleCategoriesPage() {
  ...
  const fetchCategories = () => api.get('/article-categories').then((res) => setCategories(res.data))
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await api.post('/admin/article-categories', { name: newName.trim() })
      setNewName('')
      fetchCategories()
    } catch (err) {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await api.put(`/admin/article-categories/${id}`, { name: editName.trim() })
      setEditId(null)
      fetchCategories()
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-white">文章分類管理</h2>
        <p className="text-sm text-[#6B6B70] mt-0.5">管理文章分類</p>
      </div>
      ...
      <div className="bg-[#141417] border border-[#1F1F23] rounded-xl overflow-hidden">
        {error && (
          <p className="border-b border-red-500/20 bg-red-500/10 px-5 py-3 text-sm text-red-400">{error}</p>
        )}
        {categories.length === 0 ? (
          <p className="text-sm text-[#6B6B70] text-center py-12">尚無文章分類</p>
        ) : (
